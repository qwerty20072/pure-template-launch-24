  ...
  useEffect(() => {
    const checkStripeStatuses = async () => {
      ...
      const statusPromises = teamMembers.map(async (tutor) => {
        try {
          // Check if tutor has BOTH charges_enabled and payouts_enabled for full setup
          const hasStripeSetup = tutor.charges_enabled && tutor.payouts_enabled && tutor.stripe_account_id;
